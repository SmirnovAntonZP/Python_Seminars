...
async def mes_duel(message: types.Message):
    ...
    if len(message.text.split()) > 1:  # обработка на ошибку "Если после /duel не ввели id оппонента"
        ...
        await message.answer(
            f'Введи команду /duel и укажи id оппонента через пробел или сыграй с ботом для этого жми /new_game ')
    ...
    if len(message.text.split()) > 1:  # обработка на ошибку "Если после /set не ввели количество"
        ...
        if not new_game:
            if count.isdigit():
                max_count = int(count)
                await message.answer(
                    f'Теперь конфет в игре будет {max_count}, жми /new_game или /duel и укажи id оппонента через пробел')
            else:
        ...
        await message.answer(
            f'После команды /set НАПИШИ КОЛИЧЕСТВО КОНФЕТ один раз,или же доиграй игру просто вводя количество конфет от 1 до 28')
                ...
                if message.text.isdigit() and 0 < int(message.text) < 29:
                    ...
                    if total <= 0:
                        ...
                        await message.answer(f'{name} взял {message.text} конфет. '
                                             f'На столе осталось {total}')
                        await dp.bot.send_message(enemy_id(),
                                                  f'Теперь твой ход, бери конфеты! На столе осталось ровно {total}')
                        switch_players()
                else:
                    await message.answer(f'{name}, надо указать ЧИСЛО от 1 до 28!')
